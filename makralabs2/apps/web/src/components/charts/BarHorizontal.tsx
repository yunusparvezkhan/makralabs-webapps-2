...
export function BarHorizontal({
  ...
  const scaledInsidePadding = BAR_INSIDE_PADDING_X * verticalScale;
  const scaledMinBarVerticalPadding = MIN_BAR_VERTICAL_PADDING * verticalScale;
  const scaledMaxBarVerticalPadding = MAX_BAR_VERTICAL_PADDING * verticalScale;
  ...
  const barVerticalPadding =
    data.length === 0
      ...
      : clamp(
          ...
          scaledMinBarVerticalPadding,
          scaledMaxBarVerticalPadding
        );
  const barHeight =
    data.length === 0
      ? 32
      ...
      {hasData ? (
        <g>
          {data.map((datum, index) => {
            ...
            const secondaryLabel = hideValueLabels
              ? ""
            ...
            const insidePadding = scaledInsidePadding;
